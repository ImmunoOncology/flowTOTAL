...
runFlowTOTAL <- function(fcs_path, output, panel_backgating=NULL, panel_estimate=NULL, ncores=NULL){
  ...
  runPreprocessing(metadata=metadata, output = output, report = T, ncores = ncores)
  
  message("Step2: runDensityBackgating()")
  runDensityBackgating(metadata = metadata, output = output, channel_bg = panel_backgating, ncores = ncores)
  
  message("Step3: runDensityBackgating()")
  info_panel <- read.delim(panel_estimate)
  log_file_track <- paste0(output, "/Log_file_track.txt")
  runEstimateProprotion(log_file_track, info_panel, output, ncores=ncores)
